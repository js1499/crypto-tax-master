  ...
  (function () {
    ...
    function draw() {
      ...
      for (var i = transactions.length - 1; i >= 0; i--) {
        ...
        var currentDropY =
          timelineY + (tx.priceY - timelineY) * tx.dropProgress;
    ...
    for (var p = 0; p < 25; p++) {
      ...
      var preTx = {
        ...
        priceY:
          timelineY +
          30 +
          Math.random() * (container.getBoundingClientRect().height * 0.35),
...
if (pipeline) {
  var pipeObs = new IntersectionObserver(
    function (entries) {
      if (entries[0].isIntersecting) {
...
(function () {
    ...
    function draw() {
      ...
      if (elapsed > 1500) {
        ...
        for (var i = 0; i < txCount; i++) {
          var dotX =
            fieldX + 10 + (i % 7) * ((fieldW - 20) / 7) + (fieldW - 20) / 14;
  ...
  (function () {
    ...
    function draw() {
      ...
      var gains = 80473.2 * countProgress;
      ...
      ctx.roundRect(
        padX + labelW,
        barY,
        (contentW - labelW) * 0.85 * countProgress,
        barH,
        4,
      );
      ctx.fill();
      ...
      ctx.roundRect(
        padX + labelW,
        bar2Y,
        (contentW - labelW) * 0.03 * countProgress,
        barH,
        4,
      );
      ctx.fill();
      ...
      if (elapsed > 2500) {
        ...
        var stampScale =
          0.5 + easeOut(Math.min((elapsed - 2500) / 300, 1)) * 0.5;
...
(function () {
  ...
  var prices = { solana: 83.0, ethereum: 2050.0, bitcoin: 66700.0 };
      ...
      .catch(function () {
        ...
        try {
          var ws = new WebSocket(
            "wss://ws.coincap.io/prices?assets=solana,ethereum,bitcoin",
          );
...
(function () {
  ...
  var netObs = new IntersectionObserver(
    function (entries) {
      if (entries[0].isIntersecting) {
        if (!animFrame) draw();
      } else {
...
(function () {
  ...
  function addRow() {
    ...
    row.innerHTML =
      '<span class="guarantee__ticker-check">&#10003;</span>' +
      '<span class="guarantee__ticker-hash">' +
      randomHash() +
      "</span>" +
      "<span>" +
      type +
      ...
      token +
      "</span>" +
      '<span class="guarantee__ticker-amount">$' +
  ...
  var tickerObs = new IntersectionObserver(
    function (entries) {
      if (entries[0].isIntersecting) {
        if (!running) {
          running = true;
...
(function () {
  ...
  var keyframes = [
    ...
    [7654, 147.5], // 2:07 — the trade point
  ...
  function drawSecondCandles() {
    ...
    for (var fi = searchStart; fi < searchEnd; fi++) {
      var d = Math.abs(zoomData[fi] - 147.7);
      if (d < closestDist) {
        closestDist = d;
        closestIdx = fi;
      }
...
(function () {
  ...
  canvases.forEach(function (canvas) {
    ...
    function render(seconds) {
      ...
      drawDigit(
        ctx,
        ox + (digitWidth + 2) * 2 + colonWidth + 4,
        oy,
        timeStr[3],
        green,
        border,
      ...
      drawDigit(
        ctx,
        ox + (digitWidth + 2) * 3 + colonWidth + 6,
        oy,
        timeStr[4],
        green,
        border,
    ...
    function draw() {
      ...
      switch (type) {
        ...
        case "granularity-bad":
          ...
          ctx.beginPath();
          ctx.moveTo(
            35 + 18 * Math.cos(Math.PI * 0.75),
            35 + 18 * Math.sin(Math.PI * 0.75),
          );
          ctx.lineTo(60, 60);
          ctx.stroke();
          // Lens circle
        ...
        case "costbasis-good":
          ...
          for (var b = 0; b < 3; b++) {
            ctx.fillStyle = green;
            ctx.globalAlpha =
              0.55 + b * 0.15 + Math.sin(t * 2 + b * 0.9) * 0.15;
            ctx.beginPath();
          ...
          stackDefs.forEach(function (stack, si) {
            for (var ci = 0; ci < stack.coins; ci++) {
              ...
              ctx.globalAlpha =
                0.55 +
                ci * 0.1 +
                (isTop ? Math.sin(t * 2.5 + si * 1.1) * 0.1 : 0);
              ctx.beginPath();
...
// ============================================================
(function () {
  var landingRoot = document.querySelector('[data-landing-root="true"]');
  var isAuthenticated =
    landingRoot &&
    landingRoot.getAttribute("data-landing-authenticated") === "true";
  var billingHref =
    landingRoot && landingRoot.getAttribute("data-landing-billing-href")
      ? landingRoot.getAttribute("data-landing-billing-href")
